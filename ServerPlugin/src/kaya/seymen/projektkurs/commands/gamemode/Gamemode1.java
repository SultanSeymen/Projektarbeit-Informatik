...
public class Gamemode1 implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(sender instanceof Player)
		{
			...
			if(player.hasPermission("tutorial.gm1"))
			{
				if(args.length == 0)
				{
					player.setGameMode(GameMode.CREATIVE);
					player.sendMessage("Du wurdest in den Gamemode 1 gesetzt.");
				...
				else if(args.length == 1)
				{
					...
					if(target != null)
					{
						target.setGameMode(GameMode.CREATIVE); //setze target auf gm1
						target.sendMessage("Du wurdest in den Gamemode 1 gesetzt.");
					}
				...
				{
					player.sendMessage(ChatColor.YELLOW + "Bitte benutze /gm1 <SPIELER>");
				}
			}
