...
public class Gamemode0 implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(sender instanceof Player)
		{
			...
			if(player.hasPermission("tutorial.gm0"))
			{
				if(args.length == 0)
				{
					player.setGameMode(GameMode.SURVIVAL);
					player.sendMessage("Du wurdest in den Gamemode 0 gesetzt.");
				...
				else if(args.length == 1)
				{
					...
					if(target != null)
					{
						target.setGameMode(GameMode.SURVIVAL); //setze target auf gm0
						target.sendMessage("Du wurdest in den Gamemode 0 gesetzt.");
					}
				...
				{
					player.sendMessage(ChatColor.YELLOW + "Bitte benutze /gm0 <SPIELER>");
				}
			}
